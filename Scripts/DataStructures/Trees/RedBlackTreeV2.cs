...
namespace Toolkit.DataStructures
{
	public class RedBlackTreeV2<T> : ITree<T> where T : IComparable
	{
		...
		public void ToCollection(ref ICollection<T> collection)
		{
			if (root != null)
				ToCollection(root, ref collection);
		}

		private void ToCollection(Node cur, ref ICollection<T> collection)
		{
			if (!isNil(cur))
			{
		...
		public void GreaterThan(T value, ref ICollection<T> collection)
		{
			GreaterThan(root, value, ref collection);
		}

		private void GreaterThan(Node node, T value, ref ICollection<T> collection)
		{
			if(!isNil(node))
            {
		...
		public void LessThan(T value, ref ICollection<T> collection)
		{
			LessThan(root, value, ref collection);
		}

		private void LessThan(Node node, T value, ref ICollection<T> collection)
		{
			if (!isNil(node))
			{
				if (node.value.CompareTo(value) < 0)
